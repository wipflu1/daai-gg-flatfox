import sys
import csv
def sentimentAnalysis(file_path, argNum):
    ...
    with open(file_path, 'r') as file:                      #open txt file
        ...
        for word in words:                                  #itterate words 
            with open('englishGerman.csv', 'rt') as file:   #assume englishGerman.csv = csv of german and english: word, val
    ...
    with open(descriptionOut.csv, 'w') as csvfile:
        writer = csv.writer(csvfile,fieldnames)=['description_ID','senitment_value']
        writer.writeheader()
        writer.writerows([argNum, totalSentiment])

if __name__ == "__main__":                                  #if called from sh script??
    if len(sys.argv) < 2:                                   #error if less than 1 arguement
        print("invalid number of arguments")                
        sys.exit(1)
    for index in range (1,len(sys.argv))                         #call sentimentAnalysis for every arguement provided
        sentimentAnalysis(sys.argv[index],index)
